...
d3panels.selectGroupColors = (ngroup, palette) ->
    ...
    # my own rough attempt to make a _big_ pastel palette
    pastel20 = [ "#8fc7f4", "#fed7f8", "#ffbf8e", "#fffbb8", "#8ce08c",
                 "#d8ffca", "#f68788", "#ffd8d6", "#d4a7fd", "#f5f0f5",
                 "#cc968b", "#f4dcd4", "#f3b7f2", "#f7f6f2", "#bfbfbf",
                 "#f7f7f7", "#fcfd82", "#fbfbcd", "#87feff", "#defaf5" ]
    ...
    if palette == "dark"
        ...
        return set1[0...ngroup] if ngroup <= 9
        return cat20[0...ngroup]
    else
        return ["#bebebe"] if ngroup == 1
        return ["lightpink", "lightblue"] if ngroup == 2
        return pastel1[0...ngroup] if ngroup <= 9
        return pastel20[0...ngroup]
...
d3panels.formattime = (object) ->
    ...
    "#{time[0]}:#{d3panels.padzeros(time[1])}"
