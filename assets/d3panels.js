...
d3panels.lod2dheatmap = function (chartOpts) {
  ...
  colors = (ref6 = chartOpts != null ? chartOpts.colors : void 0) != null ? ref6 : ["slateblue", "white", "crimson" // vector of three colors for the color scale (negative - zero - positive)
  ];
  ...
  chart = function chart(selection, data) {
    // (chr, pos, lod[chrx][chry])  optionally poslabel (e.g., marker names)
    var cellg, chr, i, indexWithinChr, j, k, l, len, len1, len2, m, myframe, n_pos, posByChr, ref12, ref13, ref14, tooltipfunc, x, xmid_scaled, y, ymid_scaled, zmax, zmin; // args that are lists: check that they have all the pieces
    ...
    data.chrname = d3panels.forceAsArray(data.chrname); // if equalCells, change positions to dummy values
    ...
    data = d3panels.add_chrname_start_end(data); // if equalCells, adjust chrGap to make chromosome ends equal
    ...
    myframe = d3panels.chr2dpanelframe(chartOpts); // create SVG
    ...
    svg = myframe.svg(); // scales

    xscale = myframe.xscale();
    yscale = myframe.yscale(); // split position by chromosome

    posByChr = d3panels.reorgByChr(data.chrname, data.chr, data.pos); // scaled midpoints
    ...
    zthresh = zthresh != null ? zthresh : zmin - 1; // create within-chromosome index
    ...
    tooltipfunc = function tooltipfunc(d) {
      var z;
      z = d3.format(".2f")(Math.abs(d.lod));
      return "(" + d.poslabelx + "," + d.poslabely + ") &rarr; " + z;
    };

    return celltip = d3panels.tooltip_create(d3.select("body"), cellg.selectAll("rect"), {
      tipclass: tipclass
    }, tooltipfunc);
...
d3panels.panelframe = function (chartOpts) {
  ...
  xlim = (ref13 = chartOpts != null ? chartOpts.xlim : void 0) != null ? ref13 : [0, 1 // x-axis limits
  ];
  ylim = (ref14 = chartOpts != null ? chartOpts.ylim : void 0) != null ? ref14 : [0, 1 // y-axis limits
  ];
  ...
  xscale_wnull = null;
  yscale_wnull = null; //# the main function

  chart = function chart(selection) {
    var boxes, boxes2include, d, g, i, inner_height, inner_width, xNA_xpos, xaxis, xrange, yNA_ypos, yaxis, ylabpos_x, ylabpos_y, yrange; // args that are lists: check that they have all the pieces
    ...
    svg = selection.append("svg"); // Update the dimensions

    svg.attr("width", width).attr("height", height).attr("class", "d3panels"); // put all of this stuff in a group
    ...
    yrange = [boxes.top[0] + boxes.height[0], boxes.top[0]]; // background rectangles
    ...
    g.append("g").attr("class", "title").append("text").text(title).attr("x", (width - margin.left - margin.right) / 2 + margin.left).attr("y", titlepos); // rotate y-axis title?
    ...
    yaxis.append("text").attr("class", "title").text(ylab).attr("y", ylabpos_y).attr("x", ylabpos_x).attr("transform", rotate_ylab ? "rotate(270," + ylabpos_x + "," + ylabpos_y + ")" : ""); // scales (ignoring NA business)

    xscale = d3.scaleLinear().domain(xlim).range([xrange[0] + margin.inner, xrange[1] - margin.inner]);
    yscale = d3.scaleLinear().domain(ylim).range([yrange[0] - margin.inner, yrange[1] + margin.inner]); // scales (handling nulls)

    xscale_wnull = function xscale_wnull(val) {
      if (val == null) {
        return xNA_xpos;
      }
    ...
    yscale_wnull = function yscale_wnull(val) {
      if (val == null) {
        return yNA_ypos;
      }
    ...
    }).attr("fill", "none").attr("stroke", ylineOpts.color).attr("stroke-width", ylineOpts.width).attr("shape-rendering", "crispEdges").style("pointer-events", "none"); // xlines
    ...
    }).attr("fill", "none").attr("stroke", xlineOpts.color).attr("stroke-width", xlineOpts.width).attr("shape-rendering", "crispEdges").style("pointer-events", "none"); // axis labels
...
d3panels.chrpanelframe = function (chartOpts) {
  ...
  ylim = (ref9 = chartOpts != null ? chartOpts.ylim : void 0) != null ? ref9 : [0, 1 // y-axis limits
  ];
  ...
  chart = function chart(selection, data) {
    // data = chr, start, end  (vectors with chromosome IDs, start positions, and end positions)
    var c, d, g, plot_height, plot_width, thechr, xaxis, xlines, xticks, yaxis, ylabpos_x, ylabpos_y, ylines; // args that are lists: check that they have all the pieces
    ...
    g = svg.append("g").attr("id", "frame"); // placement of boxes according to whether NAs will be treated specially
    ...
    if (horizontal) {
      // when horizontal, vertical is x-axis and horizontal is y-axis
    ...
    g.append("rect").attr("x", margin.left).attr("width", plot_width).attr("y", margin.top).attr("height", plot_height).attr("fill", rectcolor).attr("shape-rendering", "crispEdges"); // background rectangles, alternating colors
    ...
    yaxis.append("text").attr("class", "title").text(function () {
      ...
      return ylab;
    }).attr("y", ylabpos_y).attr("x", ylabpos_x).attr("transform", rotate_ylab ? "rotate(270," + ylabpos_x + "," + ylabpos_y + ")" : ""); // axis labels
...
d3panels.chr2dpanelframe = function (chartOpts) {
  ...
  chart = function chart(selection, data) {
    // data = chr, start, end  (vectors with chromosome IDs, start positions, and end positions)
    var c, chrRect, chrx, chry, g, j, k, len, len1, plot_height, plot_width, ref17, ref18, x, xaxis, y, yaxis, ylabpos_x, ylabpos_y; // args that are lists: check that they have all the pieces
...
d3panels.cichart = function (chartOpts) {
  ...
  chart = function chart(selection, data) {
    // {mean, low, high} each vectors
    var direction, high, i, low, mean, myframe, ncat, segmentGroup, tipfunc, xlim, xticks, xval, yval; // args that are lists: check that they have all the pieces
    ...
    xlim = [0.5, mean.length + 0.5]; // expand segcolor and vertsegcolor to length of mean
    ...
    myframe = d3panels.panelframe(chartOpts); // Create SVG

    myframe(selection);
    svg = myframe.svg(); // grab scale functions

    xscale = myframe.xscale();
    yscale = myframe.yscale();
    segmentGroup = svg.append("g").attr("id", "segments"); // lines from low to high
    ...
    }).attr("stroke-width", segstrokewidth).attr("shape-rendering", "crispEdges"); // lines at low, mean, and high
    ...
    }).attr("stroke-width", segstrokewidth).attr("shape-rendering", "crispEdges");
...
d3panels.crosstab = function (chartOpts) {
  ...
  chart = function chart(selection, data) {
    ...
    tab = d3panels.calc_crosstab(data); // in case labels weren't provided

    data.xlabel = (ref12 = data != null ? data.xlabel : void 0) != null ? ref12 : "";
    data.ylabel = (ref13 = data != null ? data.ylabel : void 0) != null ? ref13 : ""; // turn it into a vector of cells
    ...
    xscale = d3.scaleBand().domain(function () {
      ...
      for (var m = 0, ref16 = ncol + 1; 0 <= ref16 ? m <= ref16 : m >= ref16; 0 <= ref16 ? m++ : m--) {
        results.push(m);
      }
    ...
    yscale = d3.scaleBand().domain(function () {
      ...
      for (var m = 0, ref17 = nrow + 1; 0 <= ref17 ? m <= ref17 : m >= ref17; 0 <= ref17 ? m++ : m--) {
        results.push(m);
      }

      return results;
    }.apply(this)).range([margin.top, height - margin.bottom]); // Select the svg element, if it exists.

    svg = selection.append("svg").attr("width", width).attr("height", height).attr("class", "d3panels"); // rectangles for body of table
    ...
    }).attr("stroke-width", 0).style("pointer-events", "none").attr("shape-rendering", "crispEdges"); // text for the body of the table
    ...
    rowlab.selectAll("empty").data(data.ycat.concat("Total")).enter().append("text").attr("x", xscale(0) + cellWidth - cellPad).attr("y", function (d, i) {
      ...
      return d;
    }).attr("class", "crosstab").style("font-size", fontsize).style("pointer-events", "none"); // border around central part

    borders = svg.append("g").attr("id", "borders");
    borders.append("rect").attr("x", xscale(1)).attr("y", yscale(1)).attr("width", cellWidth * ncol).attr("height", cellHeight * nrow).attr("fill", "none").attr("stroke", bordercolor).attr("stroke-width", 2).style("pointer-events", "none").attr("shape-rendering", "crispEdges"); // border around overall total

    borders.append("rect").attr("x", xscale(ncol + 1)).attr("y", yscale(nrow + 1)).attr("width", cellWidth).attr("height", cellHeight).attr("fill", "none").attr("stroke", bordercolor).attr("stroke-width", 2).style("pointer-events", "none").attr("shape-rendering", "crispEdges"); // row and column headings and optional overall title
...
d3panels.curvechart = function (chartOpts) {
  var chart, curves, indtip, linecolor, linecolorhilit, linewidth, linewidthhilit, ref, ref1, ref2, ref3, ref4, ref5, ref6, svg, tipclass, xlim, xscale, ylim, yscale;
  ...
  chart = function chart(selection, data) {
    ...
    chartOpts.xlim = xlim;
    chartOpts.ylim = ylim; // don't allow NA boxes

    chartOpts.xNA = false;
    chartOpts.yNA = false; // set up frame
    ...
    indtip = add_curves.indtip(); // move box to front
...
d3panels.dotchart = function (chartOpts) {
  ...
  chart = function chart(selection, data) {
    // data = {x, y, indID, group} # x should be a set of positive integers; xcategories has the possible values
    var force, g, group, i, indID, jitter_width, myframe, ngroup, pointGroup, ref17, ref18, ref19, ref20, scaledPoints, u, x, xlim, xv, y; // args that are lists: check that they have all the pieces
    ...
    y = d3panels.missing2null(data.y); // grab indID if it's there
    // if no indID, create a vector of them

    indID = (ref17 = data != null ? data.indID : void 0) != null ? ref17 : function () {
      var results = [];

      for (var j = 1, ref18 = x.length; 1 <= ref18 ? j <= ref18 : j >= ref18; 1 <= ref18 ? j++ : j--) {
        results.push(j);
      }

      return results;
    }.apply(this); // a few checks
    ...
    group = function () {
      var j, len, results; // changed from (1,2,3,...) to (0,1,2,...)
    ...
    xlim = [d3.min(xcategories) - 0.5, d3.max(xcategories) + 0.5]; // whether to include separate boxes for NAs
    ...
    xscale = myframe.xscale();
    yscale = myframe.yscale(); // scaled versions of points
...
d3panels.heatmap = function (chartOpts) {
  ...
  colors = (ref4 = chartOpts != null ? chartOpts.colors : void 0) != null ? ref4 : ["slateblue", "white", "crimson" // vector of three colors for the color scale (negative - zero - positive)
  ];
  ...
  chart = function chart(selection, data) {
    // {x, y, z}  # z should be a double-indexed square matrix, z[xval][yval]
    var cell, cellrect, i, j, myframe, nx, ny, ref10, ref9, tooltipfunc, xlabels, xmid, xmid_scaled, xv, ylabels, ymid, ymid_scaled, yv, zmax, zmin; //   optionally, include xcat and ycat in place of x and y
    ...
    ymid = d3panels.calc_midpoints(d3panels.pad_vector(data.y)); // x and y axis limits

    xlim = xlim != null ? xlim : d3.extent(xmid);
    ylim = ylim != null ? ylim : d3.extent(ymid); // z-axis (color) limits; if not provided, make symmetric about 0
    ...
    zscale = d3.scaleLinear().domain(zlim).range(colors); // discard cells with |z| < zthresh
    ...
    xscale = myframe.xscale();
    yscale = myframe.yscale(); // x- and y-axis labels

    xlabels = myframe.xlabels();
    ylabels = myframe.ylabels(); // scaled x and y midpoints
    ...
    tooltipfunc = function tooltipfunc(d, i) {
      var x, y, z;
    ...
    celltip = d3panels.tooltip_create(d3.select("body"), cellrect.selectAll("rect"), {
      tipclass: tipclass
    }, tooltipfunc); // handle categorical scales:
...
d3panels.lodchart = function (chartOpts) {
  var chart, chrSelect, linecolor, linewidth, markerSelect, markertip, pointcolor, pointsize, pointstroke, ref, ref1, ref2, ref3, ref4, ref5, ref6, _svg, tipclass, _xscale, ylim, _yscale;
  ...
  _xscale = null; // x-axis scale (vector by chromosome)

  _yscale = null; // y-axis scale
  ...
  chart = function chart(selection, data) {
    ...
    data = d3panels.add_chrname_start_end(data); // organize positions and LOD scores by chromosomes

    data = d3panels.reorgLodData(data); // y-axis limits

    chartOpts.ylim = ylim != null ? ylim : [0, d3.max(data.lod) * 1.05]; // set up frame

    myframe = d3panels.chrpanelframe(chartOpts); // Create SVG
    ...
    _svg = myframe.svg(); // grab scale functions

    _xscale = myframe.xscale();
    _yscale = myframe.yscale(); // chromosome rectangles

    chrSelect = myframe.chrSelect(); // dummy chart for calling add_lodcurve()
    ...
    add2chart(self_chart, data); // grab selections

    markerSelect = add2chart.markerSelect();
    markertip = add2chart.markertip(); // move box to front
...
d3panels.add_lodcurve = function (chartOpts) {
  ...
  chart = function chart(prevchart, data) {
    ...
    svg = prevchart.svg();
    g = svg.append("g").attr("id", "lod_curve"); // grab scale functions

    xscale = prevchart.xscale();
    yscale = prevchart.yscale(); // lod curves by chr

    lodcurve = function lodcurve(chr) {
    ...
    markerSelect = hiddenpoints.selectAll("empty").data(data.markerinfo).enter().append("circle").attr("cx", function (d) {
      if (horizontal) {
        return yscale(d.lod);
      }
...
d3panels.add_curves = function (chartOpts) {
  ...
  chart = function chart(prevchart, data) {
    // prevchart = chart function used to create lodchart to which we're adding
    var curvefunc, dataByPoint, g, group, i, indID, j, k, n_ind, ngroup, ref5, ref6, ref7, svg, x, xscale, y, yscale;
    ...
    indID = (ref6 = data != null ? data.indID : void 0) != null ? ref6 : function () {
      ...
      for (var l = 1; 1 <= n_ind ? l <= n_ind : l >= n_ind; 1 <= n_ind ? l++ : l--) {
        results.push(l);
      }
    ...
    group = function () {
      var l, len, results; // changed from (1,2,3,...) to (0,1,2,...)
    ...
    linecolor = d3panels.expand2vector(linecolor, ngroup); // default dark line colors
    ...
    svg = prevchart.svg(); // grab scale functions

    xscale = prevchart.xscale();
    yscale = prevchart.yscale(); // reorganize data
...
d3panels.add_points = function (chartOpts) {
  ...
  chart = function chart(prevchart, data) {
    ...
    indID = indID != null ? indID : function () {
      ...
      for (var j = 1, ref6 = x.length; 1 <= ref6 ? j <= ref6 : j >= ref6; 1 <= ref6 ? j++ : j--) {
        results.push(j);
      }
    ...
    points = pointGroup.selectAll("empty").data(d3.range(x.length)).enter().append("circle").attr("cx", function (d, i) {
      ...
      return pointcolor[group[i]];
    }).attr("stroke", pointstroke).attr("stroke-width", "1");
    indtip = d3panels.tooltip_create(d3.select("body"), points, {
      tipclass: tipclass
    }, function (d, i) {
      return indID[i];
...
d3panels.lodheatmap = function (chartOpts) {
  ...
  colors = (ref3 = chartOpts != null ? chartOpts.colors : void 0) != null ? ref3 : ["slateblue", "white", "crimson" // vector of three colors for the color scale (negative - zero - positive)
  ];
  ...
  chart = function chart(selection, data) {
    // (chr, pos, lod) optionally: y or ycat giving scale for y-axis
    var cellg, celltipfunc, chr, direction, i, j, k, l, len, len1, len2, len3, len4, lod, m, myframe, n, n_lod, n_pos, o, pos, ref15, ref16, ref17, ref18, ref19, x, xmid_scaled, y, ylabels, ymid, ymid_scaled, yv, zmax, zmin; // also, optionally poslabel (e.g. marker name)
    ...
    data.y = function () {
      ...
      ref15 = data.y; // make sure it's numeric
    ...
    n_pos = data.chr.length;
    n_lod = data.y.length; // check inputs
    ...
    data = d3panels.reorgLodData(data); // y-axis midpoints

    ymid = d3panels.calc_midpoints(d3panels.pad_vector(data.y)); // set up frame
    ...
    xscale = myframe.xscale();
    yscale = myframe.yscale(); // y-axis labels

    ylabels = myframe.ylabels(); // scaled y-axis midpoints
    ...
    zscale = d3.scaleLinear().domain(zlim).range(colors);
    zthresh = zthresh != null ? zthresh : zmin - 1; // create cells for plotting
    ...
    celltipfunc = function celltipfunc(d) {
      var lodlabel, z;
      z = d3.format(".2f")(Math.abs(d.lod));
...
d3panels.mapchart = function (chartOpts) {
  ...
  chart = function chart(selection, data) {
    ...
    data.chrname = d3panels.forceAsArray(data.chrname);
    data.adjpos = data.pos.slice(0); // shift positions so that each chromosome starts at 0
    ...
    chrscale = function chrscale(chr) {
      var chrpos;
    ...
    }).attr("fill", "none").attr("stroke", linecolor).attr("stroke-width", linewidth).attr("shape-rendering", "crispEdges").style("pointer-events", "none"); // hash with rounded marker positions
...
d3panels.scatterplot = function (chartOpts) {
  ...
  chart = function chart(selection, data) {
    // data = {x, y, indID, group}
    var addpts, myframe, x, y; // args that are lists: check that they have all the pieces
    ...
    ylim = ylim != null ? ylim : d3panels.pad_ylim(d3.extent(y)); // set up frame
    ...
    xscale = myframe.xscale();
    yscale = myframe.yscale(); // add points
...
d3panels.trichart = function (chartOpts) {
  var boxcolor, boxwidth, chart, gridcolor, gridlines, gridwidth, height, indtip, labelpos, labels, margin, pointcolor, points, pointsize, pointstroke, pscale, rectcolor, ref, ref1, ref10, ref11, ref12, ref13, ref14, ref15, ref16, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, svg, tipclass, title, titlepos, width, xscale, yscale;
  ...
  chart = function chart(selection, data) {
    // data = {p, indID, group}
    var d, first, flag_length_not_3, flag_out_of_range, flag_sum_not_1, frame, framefunc, g, gr, group, i, indID, indices, j, len, n, ngroup, p, p1, p2, p3, p4, plot_height, plot_width, ref17, ref18, second, sum, v, vertices, vv, xlim, xy, ylim; // p = [[p00, p01, p02], [p10, p11, p12], ...]
    // args that are lists: check that they have all the pieces
    ...
    group = (ref18 = data != null ? data.group : void 0) != null ? ref18 : function () {
      var results;
      results = [];
    ...
    group = function () {
      var k, len1, results; // changed from (1,2,3,...) to (0,1,2,...)
    ...
    ylim = [0, 1]; // calculate height and width to use
    ...
    svg.attr("width", width).attr("height", height).attr("class", "d3panels"); // draw triangle
    ...
    frame.append("path").datum(indices).attr("d", framefunc).attr("fill", rectcolor).attr("stroke", boxcolor).attr("stroke-width", boxwidth); // add gridlines

    if (gridlines > 0) {
      gr = function () {
        var results = [];

        for (var k = 1; 1 <= gridlines ? k <= gridlines : k >= gridlines; 1 <= gridlines ? k++ : k--) {
          results.push(k);
        }

        return results;
      }.apply(this).map(function (i) {
        return i / (gridlines + 1);
      });

      p1 = gr.map(function (x) {
        return [x, 0, 1 - x];
      });
      p2 = gr.map(function (x) {
        return [x, 1 - x, 0];
      });
      p3 = gr.map(function (x) {
        return [0, 1 - x, x];
      });
      p4 = gr.map(function (x) {
        return [1 - x, 0, x];
      });
      first = p1.concat(p2).concat(p3);
      second = p2.concat(p3).concat(p4);
      g = frame.append("g").attr("class", "gridlines").selectAll("empty").data(first).enter().append("line").attr("x1", function (d) {
        return pscale(d).x;
      }).attr("y1", function (d) {
        return pscale(d).y;
      }).attr("x2", function (d, i) {
        return pscale(second[i]).x;
      }).attr("y2", function (d, i) {
        return pscale(second[i]).y;
      }).attr("stroke", gridcolor).attr("stroke-width", gridwidth).attr("shape-rendering", "crispEdges").style("pointer-events", "none").attr("fill", "none"); // draw outer box again

      frame.append("path").datum(indices).attr("d", framefunc).attr("fill", "none").attr("stroke", boxcolor).attr("stroke-width", boxwidth);
    } // add title


    frame.append("g").attr("class", "title").append("text").text(title).attr("x", plot_width / 2 + margin.left).attr("y", margin.top - titlepos); // add labels
    ...
    points = svg.append("g").attr("id", "points").selectAll("empty").data(p).enter().append("circle").attr("r", pointsize).attr("cx", function (d) {
      return pscale(d).x;
    }).attr("cy", function (d) {
      return pscale(d).y;
    }).attr("fill", function (d, i) {
      return pointcolor[group[i]];
    }).attr("stroke", pointstroke).attr("stroke-width", 1); // individual tooltips

    return indtip = d3panels.tooltip_create(d3.select("body"), points, {
      tipclass: tipclass
    }, function (d, i) {
      return indID[i];
    });
...
d3panels.histchart = function (chartOpts) {
  ...
  chart = function chart(selection, data) {
    ...
    ylim = ylim != null ? ylim : [0, d3panels.matrixMax(freq) * 1.05]; // set up frame
    ...
    xscale = myframe.xscale();
    yscale = myframe.yscale(); // curve data
...
d3panels.slider = function (chartOpts) {
  ...
  _chart = function chart(selection, callback, range, stops, initial_value) {
    var button, clamp_pixels, dragged, end_drag, xcscale, xscale; // args that are lists: check that they have all the pieces
    ...
    slider_svg = selection.insert("svg").attr("height", height).attr("width", width); // fully continuous x scale

    xcscale = d3.scaleLinear().range([margin.left, width - margin.right]).domain(range).clamp(true); // xscale that handles stop positions
    ...
    clamp_pixels = function clamp_pixels(pixels, interval) {
    ...
    slider_svg.selectAll("empty").data(ticks).enter().insert("text").attr("x", function (d) {
      ...
      return d;
    }).style("font-size", textsize).style("dominant-baseline", "hanging").style("text-anchor", "middle").style("pointer-events", "none").style("-webkit-user-select", "none").style("-moz-user-select", "none").style("-ms-user-select", "none"); // add scale for stops?
...
d3panels.double_slider = function (chartOpts) {
  ...
  buttondotcolor = (ref9 = chartOpts != null ? chartOpts.buttondotcolor : void 0) != null ? ref9 : ["slateblue", "orchid" // color of dot on buttons (can be vector of length 2)
  ];
  buttondotsize = (ref10 = chartOpts != null ? chartOpts.buttondotsize : void 0) != null ? ref10 : buttonsize / 4; // radius of dot on button
  ...
  _chart = function chart(selection, callback1, callback2, range, stops, initial_value) {
    var buttons, callbacks, clamp_pixels, dragged, end_drag, start_drag, xcscale, xscale; // args that are lists: check that they have all the pieces
    ...
    start_drag = function start_drag(i) {
      return function (d) {
...
d3panels.tooltip_create = function (selection, objects, options, tooltip_func) {
  ...
  in_duration = (ref3 = options != null ? options.in_duration : void 0) != null ? ref3 : 0;
  pad = (ref4 = options != null ? options.pad : void 0) != null ? ref4 : 8;
  ...
  if (fontcolor != null) {
    tipdiv.style("color", fontcolor);
  }

  if (fontsize != null) {
    tipdiv.style("font-size", fontsize + "px");
  }

  if (fill != null) {
    tridiv.style("color", fill);
  }
